...
4. Create a Git tag for releases, e.g. `v2.16.0`:

```bash
git tag -a v2.16.0 -m "2.16.0"
...
# → dist/thirdparty-blog-article-2.16.0.zip  (module under app/code + docs)
./scripts/package_module.sh --module-root
# → dist/thirdparty-blog-article-module-2.16.0.zip  (Composer-style module root)
```
