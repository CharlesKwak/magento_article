...
4. Create a Git tag for releases, e.g. `v2.18.0`:

```bash
git tag -a v2.18.0 -m "2.18.0"
...
# → dist/thirdparty-blog-article-2.18.0.zip  (module under app/code + docs)
./scripts/package_module.sh --module-root
# → dist/thirdparty-blog-article-module-2.18.0.zip  (Composer-style module root)
```
