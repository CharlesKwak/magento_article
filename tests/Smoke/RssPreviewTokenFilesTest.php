...
class RssPreviewTokenFilesTest extends TestCase
{
    ...
    {
        $root = dirname(__DIR__, 2) . '/app/code/ThirdParty/BlogArticle';
        ...
        $visibility = file_get_contents($root . '/Model/PostVisibility.php');
        $this->assertStringContainsString('PreviewToken', $visibility);
        $this->assertStringContainsString('isPreviewAllowed', $visibility);

        $router = file_get_contents($root . '/Controller/Router.php');
        $this->assertStringContainsString('PostVisibility', $router);
        $this->assertStringContainsString("getParam('preview'", $router);

        $ctrl = file_get_contents($root . '/Controller/Post/View.php');
        $this->assertStringContainsString('allowPreview', $ctrl);
        $this->assertStringContainsString('isPreviewAllowed', $ctrl);
